                	...
                	<tr>
                		<td colspan="7">
						    ...
						    <c:if test="${cri.page > 1}">
						    	<a href="adminBodApplying.jsp?page=1"><input type="button" value="처음"/></a>
						    	<c:if test="${pm.prev}">
						    		<a href="adminBodApplying.jsp?page=${pm.startPage - 1}"><input type="button" value="이전"/></a>
						    	</c:if>
						    </c:if>
						    <c:forEach var="i" begin="${pm.startPage}" end="${pm.endPage}" step="1">
						    	<a href="adminBodApplying.jsp?page=${i}"><input type="button" value="${i}"/></a>
						    </c:forEach>
						    <c:if test="${cri.page < pm.maxPage}">
						    	<c:if test="${pm.next}">
						    		<a href="adminBodApplying.jsp?page=${pm.endPage + 1}"><input type="button" value="다음"/></a>
						    	</c:if>
						    	<a href="adminBodApplying.jsp?page=${pm.maxPage}"><input type="button" value="마지막"/></a>
						    </c:if>
                		</td>
                	</tr>
                </tfoot>
